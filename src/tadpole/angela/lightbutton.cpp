...
void LightButton::paintEvent(QPaintEvent *)
{
    ...
    QPainter painter(this); //绘制准备工作,启用反锯齿,平移坐标轴中心,等比例缩放
    painter.setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing);
    if (showRect) { //绘制矩形区域
        painter.setPen(Qt::NoPen);
        painter.setBrush(bgColor);
        painter.drawRoundedRect(this->rect(), 5, 5);
        if (!text.isEmpty()) { //绘制文字
            QFont font;
        ...
        drawBorderOut(&painter); //绘制外边框
        drawBorderIn(&painter); //绘制内边框
        drawBg(&painter); //绘制内部指示颜色
        drawText(&painter); //绘制居中文字
        drawOverlay(&painter); //绘制遮罩层
    }
}
